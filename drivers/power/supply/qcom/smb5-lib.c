...
//Bug 558309, lizerong.wt, 20200522, modify, add usb_notify node
...
//+Bug 558309, lizerong.wt, 20200522, add, add usb_notify node
	...
	if( otg && (chg->usb_psy->usb_host_flag) ) {
		chg->otg_block = 1;
	    smblib_dbg(chg, PR_REGISTER, "%s: Tsejung OTG = %d, usb_host_flag = %d\n",
			    __func__, otg, chg->usb_psy->usb_host_flag);
		return;
	}
#endif
//-Bug 558309, lizerong.wt, 20200522, add, add usb_notify node
