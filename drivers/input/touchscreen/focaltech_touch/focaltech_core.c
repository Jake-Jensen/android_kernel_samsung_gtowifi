...
int fts_read_bootid(struct fts_ts_data *ts_data, u8 *id)
{
	...
	FTS_INFO("fts_read_bootid");//OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
    ...
    do {
        ...
        if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
            FTS_DEBUG("i2c read invalid, read:0x%02x%02x",
                      chip_id[0], chip_id[1]);
//+OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
        } else if((chip_id[0] == 0x58) && (chip_id[1] == 0x22)) {
    ...
    if ((cnt * INTERVAL_READ_REG) >= TIMEOUT_READ_REG) {
        ...
        if (ret <  0) {
            FTS_ERROR("read boot id fail");
             goto soft_reboot;//OA6859903, liuchunyang.wt, add, 20201021, add FT3617-003 TP IC for N8
        }
...
//+Bug613250, shenwenbin.wt, modify, 20201218, double tap have no report event 
...
static int fts_ts_remove_entry(struct i2c_client *client)
{
    struct fts_ts_data *ts_data = dev_get_drvdata(&client->dev);
    ...
    devm_tid_unregister(&client->dev, tid);
    ...
    devm_tid_and_ops_free(&client->dev, tid);
    kfree_safe(ts_data->pdata);
    kfree_safe(ts_data);
...
static void aot_enable(void *device_data)
{
    ...
    if (ret < 0) {
        input_err(true, &ts_data->client->dev, "%s: failed. ret: %d\n",
                    __func__, ret);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    tid_ops = tid->tid_ops;
    ts_data->tid = tid;
    dev_set_drvdata(&client->dev, ts_data);
	...
	FTS_ERROR("tid upgrade firmware failed");
		...
		ret = sec_cmd_init(&ts_data->sec, ft_commands,
...
{
    return fts_ts_remove_entry(client);
}
