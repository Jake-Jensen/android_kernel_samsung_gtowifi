...
static bool fts_fwupg_need_upgrade(struct fts_upgrade *upg)
{
    ...
    upg->phone_fw_version = (unsigned int)fw_ver_in_host;
        ...
        if (fw_ver_in_tp < fw_ver_in_host) { //bug613887 sheqihao.wt, modify, 2020.12.22, Touch firmware do not upgrade to low version
            return true;
        }
    } else {
...
static int tid_get_lockdown_info(struct device *dev, char *out_values)
{
    ...
    if (0x60 == vid)
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x62;//bug613689, sheqihao.wt, modify, 20201229, Modify TP panel hardware info
    else if (0x61 == vid)		
    ...
    //-Bug604581, shenwenbin.wt, add, 20201203, modify TP compatible method
        ...
        out_values[LOCKDOWN_INFO_PANEL_MAKER_INDEX] = 0x61;//bug613689, sheqihao.wt, modify, 20201229, Modify TP panel hardware info
    else if (0xda == vid)
